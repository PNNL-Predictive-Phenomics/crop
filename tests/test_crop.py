...
@pytest.mark.parametrize("medium_name,expected_growth", [
    ('glucose', True),
    ('lactose', True),  # Initially incorrect
    ('no_carbon', False)
])
...
@pytest.mark.parametrize("medium_name,expected_growth_after_fix", [
    ('glucose', True),   # Should still grow
    ('lactose', False),  # Should no longer grow
    ('no_carbon', False) # Should still not grow
])
def test_growth_after_reaction_removal(test_model, media_conditions, expected_problematic_reaction, 
...
def test_crop_algorithm_integration(test_model, media_conditions, phenotype_data, expected_problematic_reaction):
    ...
    assert lactose_solution.objective_value < 0.001, f"Error, the model should not grow on lactose after removing {suggested_removals}"
    
    # Verify other conditions still work
    glucose_model = apply_medium(model_corrected, media_conditions['glucose'])
    glucose_solution = glucose_model.optimize()
    assert glucose_solution.objective_value > 0.001, f"Error, the model should still grow on glucose after removing {suggested_removals}"
    ...
    assert no_carbon_solution.objective_value < 0.001, f"Error, the model should not grow without carbon source after removing {suggested_removals}"
...
if __name__ == "__main__":
    ...
    print(f"Lactose growth: {sol.objective_value:.4f} (should be > 0, but this is wrong!)")
